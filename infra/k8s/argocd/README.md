...
ArgoCD provides GitOps-based continuous delivery for Dhanam's Kubernetes
deployments. The live production Application is `dhanam-services`; it watches
the `infra/k8s/production/` directory on the `main` branch and automatically
...
`application.yaml` defines the repository-local Application resource. The live
cluster Application is currently generated by the platform ApplicationSet as
`dhanam-services` with the same source path and destination namespace:
...
```bash
# Check sync status
argocd app get dhanam-services

# Force sync
argocd app sync dhanam-services

# View diff without applying
argocd app diff dhanam-services

# Rollback to previous revision
argocd app rollback dhanam-services <revision>
```
