# Documentation Index

The canonical documentation map is [docs/README.md](README.md).

Use this file as a stable legacy entrypoint for tooling or bookmarks that still
open `docs/INDEX.md`.

## Primary Entry Points

- [Repository README](../README.md) - product overview and quick start
- [Development Guide](DEVELOPMENT.md) - local setup, ports, commands, tests
- [GA Remediation Roadmap](GA_REMEDIATION_ROADMAP.md) - full stability and GA program
- [Deployment Guide](DEPLOYMENT.md) - Enclii-first deployment and operations
- [Roadmap](ROADMAP.md) - stability priorities P0–P8 and milestones M1–M7
- [Commercial Stability Roadmap](COMMERCIAL_STABILITY_ROADMAP.md) - billing
  router and internal MADFAM POS completion plan
- [Tulana SKU and Catalog Handoff](TULANA_SKU_AND_CATALOG_HANDOFF_2026-05-29.md) -
  Tulana readiness, billing catalogue mirror, and Selva-approved price apply
  contract
- [Stability Audit 2026-05-19](STABILITY_AUDIT_2026-05-19.md) - current
  production/staging stability truth
- [Stability Wrap-Up 2026-05-20](STABILITY_WRAP_UP_2026-05-20.md) - latest
  production-stability status snapshot
- [Tech Debt Register](TECH_DEBT.md) - active stability and operations debt
- [Catalog Truth 2026-05-20](CATALOG_TRUTH_2026-05-20.md) - product catalog
  drift gate and production sync truth
- [Documentation Audit 2026-05-22](DOCUMENTATION_AUDIT_2026-05-22.md) -
  latest documentation corpus audit
- [Break-glass runbook](runbooks/BREAK_GLASS.md) - documented raw-access procedures
- [API index](api/README.md) - OpenAPI export and Swagger
- [API module index](../apps/api/src/modules/README.md) - NestJS module docs
- [Feature guides index](guides/README.md) - all product guides
- [Testing index](testing/README.md) - current vs archived test docs
- [Historical reports](reports/historical/README.md) - phase summaries and archived sessions
- [AI/LLM Context](../llms.txt) - compact agent context
