...
need a namespace-aware Enclii adapter.
...
| **Image pinning**                 | Digest (`sha256:...`) patched into `overlays/staging/kustomization.yaml` by CI                                | `deploy-staging.yml` patches digest-pinned API, web, and admin images and signs newly built digests.                                                                        | Aligned           | Keep staging digests reviewable in git; first signed digest refresh landed in `1af02bc2`.                                                     |
