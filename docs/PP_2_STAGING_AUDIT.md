...
`Deploy to Staging` run `26194485016` passed it for source `d1f8ccf0` before
committing staging digest refresh `7a848a2c`.
...
| **Image pinning**                 | Digest (`sha256:...`) patched into `overlays/staging/kustomization.yaml` by CI                                | `deploy-staging.yml` patches digest-pinned API, web, and admin images and signs newly built digests.                                                                                                         | Aligned        | Keep staging digests reviewable in git; latest signed refresh is `7a848a2c` for source `d1f8ccf0`.  |
...
| **ArgoCD staging Application**    | `dhanam-staging` App watches `infra/k8s/overlays/staging/`                                                    | Application is registered in `argocd`, targets `enclii-dhanam-staging`, and is Healthy/Synced at `7a848a2c`.                                                                                                 | Aligned        | Keep it reconciled by ArgoCD; do not reintroduce push-based `kubectl apply -k` from CI.             |
...
| **Staging smoke test**            | Post-deploy smoke against `staging-<domain>/health`                                                           | Run `26194485016` built/signed all images for `d1f8ccf0`, patched digests as `7a848a2c`, and passed API, web, admin, and staging API-origin smoke.                                                           | Aligned        | Keep this hosted proof required before production promotion.                                        |
