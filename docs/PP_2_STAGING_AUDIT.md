...
adds full web/admin route smoke and staging API-origin proof; the next hosted
staging run must pass it.
...
| **Image pinning**                 | Digest (`sha256:...`) patched into `overlays/staging/kustomization.yaml` by CI                                | `deploy-staging.yml` patches digest-pinned API, web, and admin images and signs newly built digests.                                                                                                         | Aligned           | Keep staging digests reviewable in git; latest signed refresh is `6717d0fb` for source `3acdeea4`.         |
...
| **ArgoCD staging Application**    | `dhanam-staging` App watches `infra/k8s/overlays/staging/`                                                    | Application is registered in `argocd`, targets `enclii-dhanam-staging`, and is Healthy/Synced at `6717d0fb`.                                                                                                 | Aligned           | Keep it reconciled by ArgoCD; do not reintroduce push-based `kubectl apply -k` from CI.                    |
...
| **Staging smoke test**            | Post-deploy smoke against `staging-<domain>/health`                                                           | Run `26189667025` built/signed all images, patched digests, and passed `staging-api.dhan.am/health`. Source now adds web/admin host smoke and staging API-origin proof.                                      | Partially aligned | Require a hosted run that passes the expanded smoke before treating staging as a complete promotion proof. |
...
| **Staging subdomain**             | `staging-<service>.<domain>` (e.g. `staging-api.dhan.am`)                                                     | DNS/custom-domain verification exists and `staging-api.dhan.am/health` passes. Source now adds web/admin host smoke and staging API-origin proof.                                                            | Partially aligned | Keep namespace-aware routes healthy and require the expanded hosted smoke to pass.                         |
