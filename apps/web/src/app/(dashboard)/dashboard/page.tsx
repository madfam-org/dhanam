...
export default function DashboardPage() {
  ...
  const { data: netWorthIndependent, isLoading: isNetWorthLoading } = useQuery({
    ...
    enabled: !!currentSpace && !isShowcaseEmbed,
  ...
  return (
    <div
      className={`${isShowcaseEmbed ? 'space-y-3 overflow-hidden' : 'space-y-6'}`}
      aria-live="polite"
      aria-busy={isLoading}
    >
      ...
      {!isShowcaseEmbed && isNonGuestDemo && <InsightCards />}

      {/* Savings Streak for non-guest demo personas */}
      {!isShowcaseEmbed && isNonGuestDemo && <SavingsStreak />}

      {/* Meet the Personas for guest users */}
      {!isShowcaseEmbed && isGuestDemo && <MeetThePersonas />}

      {/* Key Metrics */}
      <div
        className={
          isShowcaseEmbed ? 'grid grid-cols-2 gap-2' : 'grid gap-4 md:grid-cols-2 lg:grid-cols-4'
        }
      >
        ...
        {!isShowcaseEmbed && !isLoading && goals && goals.length > 0 && (
        ...
        <>
          ...
          {!isLoading && (
            <InsightsCard
              netWorth={netWorth}
              previousNetWorth={
                netWorthData?.changeAmount != null
                  ? netWorth - netWorthData.changeAmount
                  : undefined
              }
                    ...
                    {accounts?.map((account) => {
                      ...
                      return (
                        <div key={account.id} className="flex items-center">
                          ...
                          <div
                            className={`font-medium ${account.balance < 0 ? 'text-red-600' : ''}`}
                          >
                            {account.balance < 0 && (
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <p className="text-sm text-muted-foreground">
                        {t('overview.projectedIncome')}
                      </p>
                      <p className="text-lg font-semibold text-green-600">
                        {formatCurrency(
                          cashflowForecast.summary.totalIncome,
                          currentSpace.currency
                        )}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">
                        {t('overview.projectedExpenses')}
                      </p>
                      <p className="text-lg font-semibold text-red-600">
                        {formatCurrency(
                          cashflowForecast.summary.totalExpenses,
                          currentSpace.currency
                        )}
                      </p>
                    </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span>
                        {formatCurrency(
                          cashflowForecast.summary.currentBalance,
                          currentSpace.currency
                        )}
                      </span>
                    </div>
